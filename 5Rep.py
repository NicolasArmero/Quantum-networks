...
def example_network_setup(node_distance, depol):
	...
	nodo1 = Node("1", qmemory=create_qprocessor("mem_1"))
	nodo2 = Node("2", qmemory=create_qprocessor("mem_2"))
	
	channel_0A = QuantumChannel(name="qchannel02A", length=node_distance, models={"fibre_delay_model": FibreDelayModel(), "quantum_loss_model": FibreLossModel(p_loss_init=0, p_loss_length=0, rng=np.random.RandomState(42))})
	channel_0A.models['quantum_noise_model'] = FibreDepolarizeModel(p_depol_init=0, p_depol_length=depol)
	channel_A0 = QuantumChannel(name="qchannelA20", length=node_distance, models={"fibre_delay_model": FibreDelayModel(), "quantum_loss_model": FibreLossModel(p_loss_init=0, p_loss_length=0, rng=np.random.RandomState(42))})
	channel_A0.models['quantum_noise_model'] = FibreDepolarizeModel(p_depol_init=0, p_depol_length=depol)
	conn = DirectConnection(name="conn0A",	channel_AtoB=channel_0A, channel_BtoA=channel_A0)
	
	channel_B1 = QuantumChannel(name="qchannel02A", length=node_distance, models={"fibre_delay_model": FibreDelayModel(), "quantum_loss_model": FibreLossModel(p_loss_init=0, p_loss_length=0, rng=np.random.RandomState(42))})
	channel_B1.models['quantum_noise_model'] = FibreDepolarizeModel(p_depol_init=0, p_depol_length=depol)
	channel_1B = QuantumChannel(name="qchannelA20", length=node_distance, models={"fibre_delay_model": FibreDelayModel(), "quantum_loss_model": FibreLossModel(p_loss_init=0, p_loss_length=0, rng=np.random.RandomState(42))})
	channel_1B.models['quantum_noise_model'] = FibreDepolarizeModel(p_depol_init=0, p_depol_length=depol)
	conn1 = DirectConnection(name="conn0A",	channel_AtoB=channel_B1, channel_BtoA=channel_1B)
	
	channel_12 = QuantumChannel(name="qchannel1to2", length=node_distance, models={"fibre_delay_model": FibreDelayModel(), "quantum_loss_model": FibreLossModel(p_loss_init=0, p_loss_length=0, rng=np.random.RandomState(42))})
	channel_12.models['quantum_noise_model'] = FibreDepolarizeModel(p_depol_init=0, p_depol_length=depol)
	channel_21 = QuantumChannel(name="qchannel2to1", length=node_distance, models={"fibre_delay_model": FibreDelayModel(), "quantum_loss_model": FibreLossModel(p_loss_init=0, p_loss_length=0, rng=np.random.RandomState(42))})
	channel_21.models['quantum_noise_model'] = FibreDepolarizeModel(p_depol_init=0, p_depol_length=depol)
	conn2 = DirectConnection(name="conn12",	channel_AtoB=channel_12, channel_BtoA=channel_21)
	
	network.add_nodes([nodo0, alice, bob, rep, nodo1, nodo2])
	...
	port_b1, port_1b = network.add_connection(bob, nodo1, connection=conn1, label="quantum", port_name_node1="qubitOUT", port_name_node2="qubitIN")
	
	port_12, port_21 = network.add_connection(nodo1, nodo2, connection=conn2, label="quantum", port_name_node1="qubitOUT", port_name_node2="qubitIN")
...
def setup_repeater_protocol(network):
	...
	nodo1 = network.get_node("1")
	nodo2 = network.get_node("2")
	...
	protocol.add_subprotocol(subprotocol)
	subprotocol = MedioProtocol(nodo1)
	protocol.add_subprotocol(subprotocol)
	subprotocol = FinProtocol(nodo2)
	protocol.add_subprotocol(subprotocol)
...
class MedioProtocol(NodeProtocol):
...
class FinProtocol(NodeProtocol):
	
	def __init__(self, node):
		super().__init__(node)
		#port_bob = self.node.ports["qubitIN"]
...
def setup_datacollector(network, protocol, num_runs):
	...
	dc.collect_on(py.EventExpression(source=protocol.subprotocols['FinProtocol'], event_type=Signals.SUCCESS.value))
	return dc
...
def create_plot(num_runs):
	...
	for depolar in [0, 0.001]:
		...
		print(f"Despolarización: {depolar}")
		print(f"{data}")
		print("")
