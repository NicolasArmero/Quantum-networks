...
def setup_datacollector(network, protocol):
    ...
    def calc_fidelity(evexpr):
    	...
    	qubit = qubits[0]
    	nodes[0].qmemory.put(qubits=qubit, positions=1, replace=True)
    	measure_program = BellMeasurementProgram()
    	yield nodes[0].qmemory.execute_program(measure_program)
    	m1, = measure_program.output["M1"]
    	m2, = measure_program.output["M2"]
    	if m1 == 1:
    		nodes[-1].qmemory.execute_instruction(INSTR_Z)
    		yield self.await_program(nodes[-1].qmemory)
    	if m2 == 1:
