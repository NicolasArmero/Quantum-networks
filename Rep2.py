...
def create_qprocessor(name):
    ...
    measure_noise_model = DephaseNoiseModel(dephase_rate=0.1, time_independent=True)
...
def setup_network(num_nodes, node_distance, source_frequency, att, depol):
    ...
    clas = ns.examples.teleportation.ClassicalConnection(name=f"con B-A", length=node_distance*(num_nodes-1))
    network.add_connection(nodes[-1], nodes[0], connection=clas, label="classical", port_name_node1="cout_bob", port_name_node2="cin_alice")
...
class CorrectionProtocol(NodeProtocol):
    ...
    def run(self):
        ...
        port_charlie = self.node.qmemory.ports["qin0"]
        entanglement_ready = False
        meas_results = None
        while True:
...
def example_sim_setup(node_A, node_B):
    ...
    protocol_alice = CorrectionProtocol(node_A)
    dc = DataCollector(collect_fidelity_data, include_entity_name=False)
    dc.collect_on(py.EventExpression(source=protocol_alice, event_type=Signals.SUCCESS.value))
    return protocol_alice, dc
...
def run_simulation(num_nodes=4, node_distance=20, num_iters=100, att=0.2, depol=0.07):
    ...
    protocol_alice, dc = example_sim_setup(nodeA, nodeB)
...
def create_plot(num_iters):
    ...
    depolar = [0, 0.01, 0.02, 0.03, 0.05, 0.07, 0.1, 0.15, 0.2]
