...
def setup_network(num_nodes, node_distance, source_frequency, att):
...
class EntanglingConnection(Connection):
    ...
    def __init__(self, length, source_frequency, att, name="EntanglingConnection"):
        ...
        #loss_model = FibreAqsourcettenuation(p_loss_init=1, p_loss_length=att)
        ...
        qchannel_c2a = QuantumChannel("qchannel_C2A", length=length / 2, models={"quantum_loss_model": FibreLossModel(p_loss_init=0, p_loss_length=att, rng=None)})
        qchannel_c2b = QuantumChannel("qchannel_C2B", length=length / 2, models={"quantum_loss_model": FibreLossModel(p_loss_init=0, p_loss_length=att, rng=None)})
...
def time_limit(seconds):
	def signal_handler(signum, frame):
		raise TimeoutException("¡Tiempo!")
	sg.signal(sg.SIGALRM, signal_handler)
	sg.alarm(seconds)
	try:
		yield
	finally:
...
def run_simulation(num_nodes=4, node_distance=20, num_iters=100, att=0.2):
    ...
    network = setup_network(num_nodes, node_distance=node_distance, source_frequency=1e9 / est_runtime, att=att)
    global protocol
...
def create_plot(num_iters):
    ...
    fig, ax = plt.subplots()
    for att in [0.1, 0.15, 0.2, 0.3, 0.5]:
        data = pd.DataFrame()
        for distance in [2, 5, 10, 15, 20, 25, 30, 40, 50, 100]:
            data[distance] = run_simulation(num_nodes=3, node_distance=distance / 2, num_iters=num_iters, att=att)['fidelity']
            #try:
            	#data[distance] = run_simulation(num_nodes=3, node_distance=distance / 2, num_iters=num_iters, att=att)['fidelity']
            #except:
            	#fallo = 1
        ...
        data.plot(y='fidelity', yerr='sem', label=f"{att}", ax=ax)
