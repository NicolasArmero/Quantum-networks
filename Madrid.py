...
def run_simulation(depol):
	...
	dc = setup_datacollector(network, f_protocol)
	a_protocol.start()
	b_protocol.start()
	c_protocol.start()
	d_protocol.start()
...
def create_plot(num_rep=100):
	...
	for depol in depolar:
		data[depol] = run_simulation(depol=depol)['fidelity']
	data = data.agg(['mean', 'sem']).T.rename(columns={'mean': 'fidelity'})
	data.plot(y='fidelity', yerr='sem', label="Comunicación A-F", ax=ax)
	print(f"{data}")
	print("")
