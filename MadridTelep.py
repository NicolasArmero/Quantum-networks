...
def create_network(depol, telep):
	...
	if telep == 1:
		...
		port_bc, port_cb = network.add_connection(node_B, node_C, connection=conBC, label="quantum", port_name_node1="qubitOUT", port_name_node2="qubitIN")
		node_C.ports[port_cb].forward_input(node_C.qmemory.ports['qin0'])
	
		# Setup entangling connection between nodes:
		source_frequency = 4e4 / 33.1
		q_conn = EntanglingConnection(length=33.1, source_frequency=source_frequency)
		for channel_name in ['qchannel_C2A', 'qchannel_C2B']:
			q_conn.subcomponents[channel_name].models['quantum_noise_model'] = FibreDepolarizeModel(p_depol_init=0, p_depol_length=depol)
		port_cc, port_dc = network.add_connection(node_C, node_D, connection=q_conn, label="quantum", port_name_node1="qin_charlie", port_name_node2="qin_charlie")
		node_C.ports[port_cc].forward_input(node_C.qmemory.ports['qin1'])
		node_D.ports[port_dc].forward_input(node_D.qmemory.ports['qin0'])
...
class Protocol1(NodeProtocol):
	...
	def run(self):
		while True:
			if self.telep:
				yield self.await_port_input(self.node.ports["cin_D"])
...
def run_simulation(depol, num_rep, telep):
	...
	if telep == 0:
		...
		c_protocol = BellMeasurementProtocol(network.get_node("C"))
		d_protocol = CorrectionProtocol(network.get_node("D"))
