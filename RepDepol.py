...
def setup_network(num_nodes, node_distance, source_frequency, depol):
    ...
    for i in range(num_nodes - 1):
        ...
        qconn = EntanglingConnection(name=f"qconn_{i}-{i+1}", length=node_distance, source_frequency=source_frequency, att=0.1)
...
def setup_datacollector(network, protocol):
    ...
    def calc_fidelity(evexpr):
        ...
        global num
        fidelity = ns.qubits.fidelity([qubit_a, qubit_b], ns.qubits.ketstates.b00, squared=False)
        num += 1
...
num = 0    
def run_simulation(num_nodes=4, node_distance=20, num_iters=100, depol=0.02):
    ...
    network = setup_network(num_nodes, node_distance=node_distance, source_frequency=1e9 / est_runtime, depol=depol)
...
def create_plot(num_iters):
    ...
    for nodos in [3, 4]:
    	for depol in [0.001, 0.01, 0.025, 0.05, 0.07, 0.1]:
        	data = pd.DataFrame()
        	for distance in [2, 5, 10, 15, 20, 30, 40, 50, 75, 100]:
        		fallo = 1
        		while fallo ==1:
        			try:
        				data[distance] = run_simulation(num_nodes=nodos, node_distance=distance/(nodos-1), num_iters=num_iters, depol=depol)['fidelity']
        				fallo = 0
        			except:
        				fallo = 1
            		
        	data = data.agg(['mean', 'sem']).T.rename(columns={'mean': 'fidelity'})
        	data.plot(y='fidelity', yerr='sem', label=f"{depol} "f"{nodos-2}", ax=ax)
    ...
    plt.title("Fidelidad en función de la depolarización y del número de repetidores")
    plt.show()
