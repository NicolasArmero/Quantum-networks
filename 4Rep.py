...
def example_network_setup(node_distance, depol):
	...
	network.add_nodes([nodo0, alice, bob, rep, nodo1])
...
class UnoProtocol(NodeProtocol):
...
def setup_datacollector(network, protocol, num_runs):
	...
	dc.collect_on(py.EventExpression(source=protocol.subprotocols['UnoProtocol'], event_type=Signals.SUCCESS.value))
	return dc
