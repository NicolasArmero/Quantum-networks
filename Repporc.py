...
class EntanglingConnection(Connection):
    ...
    def __init__(self, length, source_frequency, att, name="EntanglingConnection"):
        ...
        qchannel_c2a = QuantumChannel("qchannel_C2A", length=0, models={"delay_model": FibreDelayModel()})
        qchannel_c2b = QuantumChannel("qchannel_C2B", length=length, models={"delay_model": FibreDelayModel(), "quantum_loss_model": FibreLossModel(p_loss_init=0, p_loss_length=att, rng=np.random.RandomState(42))})
...
def create_plot(num_iters):
    ...
    for nodos in [3, 5]:
    	for att in [0.2, 0.25, 0.3, 0.35, 0.4]:
    		...
    		for distance in dist:
    			...
    			while fallo == 1:
    				try:
    					data[distance] = run_simulation(num_nodes=nodos, node_distance=distance/(nodos-1), num_iters=num_iters, att=att)['fidelity']
    					porcentaje = (num/(num_iters))*100
    					print(f"Distancia: {distance} "f"Atenuación: {att} "f"Recibidos: {num} "f"Porcentaje: {porcentaje}")
        ...
        #print("Atenuación: "f"{att}")
    		plt.plot(dist, porc, 'o-', label=f"{att} dB/km - "f"{nodos-2}")
    		#print(f"Nodos: {nodos}")
    		#print(f"{data}")
    		#print("")
    ...
    plt.title("Porcentaje de qubits recibidos")
    plt.show()
