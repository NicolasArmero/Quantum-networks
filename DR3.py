...
def create_plot(num_iters):
    ...
    rep = num_iters
    data = pd.DataFrame()
    for depol in [0.01, 0.015, 0.025, 0.05, 0.07, 0.1]:
    	if depol == 0.01:
    		data[depol] = directa(distance=20, att=0.25, depol=0.01)['fidelity']
    	else:
    		data[depol] = data[0.01]
    data = data.agg(['mean', 'sem']).T.rename(columns={'mean': 'fidelity'})
    data.plot(y='fidelity', yerr='sem', label="0.01 - Directa", ax=ax)
    #dist = [i for i in range(40, 60, 2)]
    for nodos in [3, 4, 5, 6, 7, 8]:
    	data = pd.DataFrame()
    	for depol in [0.01, 0.015, 0.025, 0.05, 0.07, 0.1]:
    		data[depol] = repetidores(num_nodes=nodos, node_distance=20/(nodos-1), num_iters=num_iters, att=0.25, depol=depol)['fidelity']
    		num = 0
    	data = data.agg(['mean', 'sem']).T.rename(columns={'mean': 'fidelity'})
    	if nodos != 2:
    ...
    plt.grid()
    plt.legend(title='Despolarización y número de repetidores:')
    plt.title("Despolarización")
    plt.show()
