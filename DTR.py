...
#CÓDIGO DE LA TRANSMISIÓN CON TELEPORTACIÓN
...
class ClassicalConnection(Connection):
	#Creación de conexiones clásicas
...
class BellMeasurementProtocol(NodeProtocol):
    ...
    def run(self):
        ...
        qubit_init_program = ns.examples.teleportation.InitStateProgram()
        measure_program = ns.examples.teleportation.BellMeasurementProgram()
...
def example_network_repeater(node_distance, depol):
...
def setup_repeater_protocol(network):
	...
	subprotocol = BellMeasurementProtocolR(alice)
	protocol.add_subprotocol(subprotocol)
	# Add CorrectionProtocol to Bob
	subprotocol = CorrectionProtocolR(bob)
	protocol.add_subprotocol(subprotocol)
	return protocol
...
class InitStateProgramR(QuantumProgram):
...
class BellMeasurementProgramR(QuantumProgram):
...
class BellMeasurementProtocolR(NodeProtocol):
	
	def run(self):
		entanglement_ready = False
		swapping_ended = False
		iniciado = False
		qubit_init_program = InitStateProgramR()
		measure_program = BellMeasurementProgramR()
...
class CorrectionProtocolR(NodeProtocol):
...
def setup_datacollectorR(network, protocol, num_runs):
	...
	dc.collect_on(py.EventExpression(source=protocol.subprotocols['CorrectionProtocolR'], event_type=Signals.SUCCESS.value))
	return dc
	
	
def repetidores(distance, depolar, num_runs):
	
	ns.sim_reset()
	est_runtime = (0.5 + 3 - 1) * distance * 5e3
	network = example_network_repeater(distance, depolar)
	protocol = setup_repeater_protocol(network)
	dc = setup_datacollectorR(network, protocol, num_runs)
	protocol.start()
	ns.sim_run(end_time=10e9, magnitude=1e9)
	return dc.dataframe
...
def create_plot(num_iters):
    		...
    		for depol in [0.05, 0.07, 0.1]:
    			...
    			for distance in dist:
    				...
    				#while fallo == 1:
    					#try:
    				if nodos == 2:
    					data[distance] = directa(distance=distance, att=att, depol=depol)['fidelity']
    				elif nodos == 1:
    					data[distance] = run_experiment(num_runs=num_iters, distance=distance, depolar_rate=depol, dephase_rate=0.0)['fidelity']
    				else:
    					data[distance] = repetidores(distance=distance, depolar=depol, num_runs=num_iters)['fidelity']
    				num = 0
            		
    			data = data.agg(['mean', 'sem']).T.rename(columns={'mean': 'fidelity'})
    			if nodos > 2:
    				#data['fidelity'] = data['fidelity']*(2/3) + 1/3
    				data.plot(y='fidelity', yerr='sem', label=f"{att} dB/km - "f"{depol} - "f"{nodos-2} rep", ax=ax)
    			elif nodos == 2:
    				data.plot(y='fidelity', yerr='sem', label=f"{att} dB/km - "f"{depol} - Directa", ax=ax)
    			elif nodos == 1:
    				data.plot(y='fidelity', yerr='sem', label=f"{att} dB/km - "f"{depol} - Teleportación", ax=ax)
    			print(f"Nodos: {nodos}")
    ...
    plt.title("Comparación de las tres formas de conexión")
    plt.show()
